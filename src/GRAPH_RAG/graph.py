...
def create_graph(config: ConfigGraph) -> StateGraph:
    ...
    # Define the nodes
    graph.add_node("query_classificator", lambda state: query_classificator(
        state=state, agent=query_classifier, get_chain=get_chain))
    graph.add_node("retriever", lambda state: retriever(
        state=state, vector_database=vector_db))
    graph.add_node("retreived_docs_grader", lambda state: retreived_docs_grader(
        state=state, agent=docs_grader, get_chain=get_chain))
    graph.add_node("reprocess_query", lambda state: process_query(
        state=state, agent=query_processor, get_chain=get_chain))
    graph.add_node("generator", lambda state: generator(
        state=state, agent=generator_agent, get_chain=get_chain))
    graph.add_node("hallucination_checker", lambda state: hallucination_checker(
        state=state, agent=hallucination_grader, get_chain=get_chain))
    graph.add_node("generation_grader", lambda state: generation_grader(
        state=state, agent=answer_grader, get_chain=get_chain))
...
def save_graph(compile_graph) -> None:
    ...
    figure_path = os.path.join(os.path.dirname(__file__), "..", "..", "data", "figures",
                               "graphs", f'{get_current_spanish_date_iso_file_name_format()}_graph.png')
    ...
    try:
        with open(figure_path, 'wb') as f:
            f.write(compile_graph.get_graph().draw_mermaid_png(
